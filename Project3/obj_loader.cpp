...
OBJModel::OBJModel(const std::string& fileName)
{
  ...
  if (file.is_open())
  {
    while (file.good())
    {
      ...
      unsigned int lineLength = static_cast<unsigned int>(line.length());
...
void IndexedModel::print() const
{
  ...
  index < positions.size();
    index++)
  {
    std::cout << "X:" << positions[index].x << " Y:" << positions[index].y << " Z:" << positions[index].z << " ";
  }
  std::cout << "texCoords\n";
  for (int index = 0;
  index < texCoords.size();
    index++)
  {
    std::cout << "X:" << texCoords[index].x << " Y:" << texCoords[index].y;
  }
  std::cout << "normals\n";
  for (int index = 0;
  index < normals.size();
    index++)
  {
    std::cout << "X:" << normals[index].x << " Y:" << normals[index].y << " Z:" << normals[index].z << " ";
  }
  std::cout << "indices\n";
  for (int index = 0;
  index < indices.size();
...
IndexedModel OBJModel::ToIndexedModel()
{
  ...
  unsigned int numIndices = static_cast<unsigned int>(OBJIndices.size());
  ...
  for (unsigned int i = 0; i < numIndices; i++)
  {
    ...
    {
      normalModelIndex = static_cast<unsigned int>(normalModel.positions.size());
    ...
    {
      resultModelIndex = static_cast<unsigned int>(result.positions.size());
...
unsigned int OBJModel::FindLastVertexIndex(const std::vector<OBJIndex*>& indexLookup, const OBJIndex* currentIndex, const IndexedModel& result)
{
  ...
  unsigned int end = static_cast<unsigned int>(indexLookup.size());
